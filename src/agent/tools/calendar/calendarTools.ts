...
async function sendMeetingEmail(clientEmail: string, clientName: string, meetingDetails: any) {
  const mailOptions = {
...
function formatDateTimeForGoogle(dateStr: string, timeStr: string) {
  const [year, month, day] = dateStr.split("-").map((num) => Number.parseInt(num))
  const [hours, minutes] = timeStr.split(":").map((num) => Number.parseInt(num))
  const date = new Date(Date.UTC(year, month - 1, day, hours, minutes, 0))
  return date.toISOString()
}

// Determine meeting duration based on meeting type
function getMeetingDurationMinutes(meetingType: string) {
        ...
        if (currentDate.getDay() !== 0 && currentDate.getDay() !== 6) {
          ...
          for (let hour = businessStartHour; hour < businessEndHour; hour++) {
            ...
            const slotStart = new Date(
              `${dateString}T${startTime}:00.000${timeZone === "America/Los_Angeles" ? "-08:00" : "+00:00"}`,
            )
            const slotEnd = new Date(
              `${dateString}T${endTime}:00.000${timeZone === "America/Los_Angeles" ? "-08:00" : "+00:00"}`,
            )
  ...
  async ({
    ...
    try {
      ...
      const useVideoConference =
        location.toLowerCase().includes("google meet") || location.toLowerCase().includes("zoom")
...
Automatically scheduled by Lisa, your personal assistant.
      `.trim()
      ...
      if (!response.data) {
        throw new Error("Failed to get response data from calendar API");
      }

      const meetingDetails = {
        subject: eventResource.summary,
        with: clientName,
        date,
  ...
  },
  {
    ...
    schema: z.object({
      ...
      location: z
        .string()
        .optional()
        .describe("Meeting location or 'Google Meet' for video conference (default: Google Meet)"),
      timeZone: z.string().optional().describe("Time zone (default: America/Los_Angeles)"),
    }),
  },
)
  ...
  async ({
    ...
    try {
      ...
      if (!existingEvent.data) {
        throw new Error("Failed to get existing event data");
      }
      ...
      const durationMinutes = Math.round((existingEnd.getTime() - existingStart.getTime()) / 60000)
...
const cancelMeeting = tool(
  async ({ meetingId, reason = "Cancellation requested", sendNotification = true }) => {
    try {
      ...
      const formattedEvents = events.map((event) => {
        ...
        return {
          ...
          attendees: event.attendees
            ? event.attendees.map((a) => ({ email: a.email, name: a.displayName || a.email }))
            : [],
...
const calendarTools = [checkAvailability, scheduleMeeting, rescheduleMeeting, cancelMeeting, getUpcomingMeetings]
